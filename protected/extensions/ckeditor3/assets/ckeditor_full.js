...
(function() {
    ...
    a.htmlParser.fragment = function() {
        ...
        a.htmlParser.fragment.fromHtml = function(p, q, r) {
            var s = new a.htmlParser(),
            t = r || new a.htmlParser.fragment(),
            u = [],
            v = [],
            w = t,
            x = false;
            ...
            function A(B, C, D) {
                ...
                if (q && (!C.type || C.name == 'body')) {
                    var F, G;
                    if (B.attributes && (G = B.attributes['data-cke-real-element-type'])) F = G;
                    else F = B.name;
                ...
                if (B._.isBlockLike && B.name != 'pre') {
                    ...
                    I = B.children[H - 1],
                    J;
                    if (I && I.type == 3) if (! (J = e.rtrim(I.value))) B.children.length = H - 1;
                    else I.value = J;
                }
                C.add(B);
                if (B.returnPoint) {
            ...
            s.onTagOpen = function(B, C, D, E) {
                ...
                while (1) {
                    var G = w.name,
                    H = G ? f[G] || (w._.isBlockLike ? f.div: f.span) : o;
                    if (!F.isUnknown && !w.isUnknown && !H[B]) {
                        if (w.isOptionalClose) s.onTagClose(G);
                        else if (B in n && G in n) {
                            var I = w.children,
                            J = I[I.length - 1];
                            if (! (J && J.name == 'li')) A(J = new a.htmlParser.element('li'), w); ! F.returnPoint && (F.returnPoint = w);
                            w = J;
                        } else if (B in f.$listItem && G != B) s.onTagOpen(B == 'li' ? 'ul': 'dl', {},
                        0, 1);
                        else if (G in l && G != B) { ! F.returnPoint && (F.returnPoint = w);
                            w = w.parent;
                        } else {
                            if (G in f.$inline) u.unshift(w);
                            if (w.parent) A(w, w.parent, 1);
                            else {
                                F.isOrphan = 1;
                                break;
                            }
            ...
            s.onText = function(B) {
                ...
                z();
                y();
                if (q && (!w.type || w.name == 'body') && e.trim(B)) this.onTagOpen(q, {},
                0, 1);
                if (!x) B = B.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, ' ');
    ...
    e.extend(i, {
        ...
        j.list = {
            listToArray: function(E, F, G, H, I) {
                ...
                K = E.getChildCount(); J < K; J++) {
                    ...
                    G.push(M);
                    for (var N = 0,
                    O = L.getChildCount(), P; N < O; N++) {
                        P = L.getChild(N);
                        if (P.type == 1 && m[P.getName()]) j.list.listToArray(P, F, G, H + 1, M.grandparent);
                        else M.contents.push(P);
                    }
                }
                return G;
            },
            arrayToList: function(E, F, G, H, I) {
                ...
                var J = E[G].parent.getDocument(),
                K = new d.documentFragment(J),
                L = null,
                M = G,
                N = Math.max(E[G].indent, 0),
                O = null,
                P,
                Q = H == 1 ? 'p': 'div';
                while (1) {
                    ...
                    if (R.indent == N) {
                        ...
                        if (P != L.getDirection(1)) O.setAttribute('dir', P);
                        ...
                        var T = E[M - 1].element.getDirection(1),
                        U = j.list.arrayToList(E, null, M, H, T != P ? P: null);
                        if (!O.getChildCount() && c && !(J.$.documentMode > 7)) O.append(J.createText('\xa0'));
                        O.append(U.listNode);
                        M = U.nextIndex;
                    } else if (R.indent == -1 && !G && R.grandparent) {
                        if (m[R.grandparent.getName()]) O = R.element.clone(false, true);
                        else if (I || R.element.hasAttributes() || H != 2) {
                            O = J.createElement(Q);
                            R.element.copyAttributes(O, {
                                type: 1,
                                value: 1
                            });
                            if (!I && H == 2 && !O.hasAttributes()) O = new d.documentFragment(J);
                        } else O = new d.documentFragment(J);
                        if (O.type == 1) if (R.grandparent.getDirection(1) != P) O.setAttribute('dir', P);
                        else O.removeAttribute('dir');
        ...
        function s(E, F, G, H) {
            var I = j.list.listToArray(F.root, G),
            J = [];
            ...
            var M = F.root,
            N = M.getDocument().createElement(this.type);
            M.copyAttributes(N, {
                start: 1,
                type: 1
            });
            ...
            var P = j.list.arrayToList(I, G, null, E.config.enterMode),
            Q,
            R = P.listNode.getChildCount();
        ...
        function u(E, F, G) {
            ...
            var N = E.config.useComputedState,
            O, P;
            N = N === undefined || N;
            ...
            var T = J[J.length - 1].getNext(),
            U = I.createElement(this.type);
            G.push(U);
            var V, W;
            while (J.length) {
                V = J.shift();
                W = I.createElement('li');
                if (V.is('pre') || t.test(V.getName())) V.appendTo(W);
                else {
                    V.copyAttributes(W);
                    if (O && V.getDirection()) {
            ...
            if (T) U.insertBefore(T);
            else U.appendTo(L);
        };
        function v(E, F, G) {
            var H = j.list.listToArray(F.root, G),
            I = [];
            ...
            for (J = L + 1; J < H.length; J++) {
                if (H[J].indent > H[J - 1].indent + 1) {
                    var N = H[J - 1].indent + 1 - H[J].indent,
                    O = H[J].indent;
            ...
            var P = j.list.arrayToList(H, G, null, E.config.enterMode, F.root.getAttribute('dir')),
            Q = P.listNode,
            R,
            S;
            function T(U) {
                if ((R = Q[U ? 'getFirst': 'getLast']()) && !(R.is && R.isBlockBoundary()) && (S = F.root[U ? 'getPrevious': 'getNext'](d.walker.whitespaces(true))) && !(S.is && S.isBlockBoundary({
                    br: 1
            ...
            exec: function(E) {
                ...
                while ((K = P.getNextRange()) && ++Q) {
                    ...
                    while (V = U.getNextParagraph()) {
                        ...
                        if (aa) continue;
                        var af = ab;
                        if (af.getCustomData('list_group_object_' + Q)) af.getCustomData('list_group_object_' + Q).contents.push(V);
                        else {
                            ae = {
                                root: af,
                                contents: [V]
                ...
                while (N.length > 0) {
                    ae = N.shift();
                    if (this.state == 2) {
                        if (m[ae.root.getName()]) s.call(this, E, ae, O, ag);
                        else u.call(this, E, ae, ag);
                    } else if (this.state == 1 && m[ae.root.getName()]) v.call(this, E, ae, O);
                }
                for (ad = 0; ad < ag.length; ad++) {
                    Z = ag[ad];
                    var ah, ai = this; (ah = function(aj) {
        ...
        function A(E) {
            return function(F) {
                ...
                if (J && (J.name && J.name == 'br' || J.value && (K = J.value.match(y)))) {
                    var L = J;
                    if (! (K && K.index) && L == G[0]) G[0] = E || c ? new a.htmlParser.text('\xa0') : new a.htmlParser.element('br', {});
                    else if (L.name == 'br') G.splice(H - 1, 1);
                    else L.value = L.value.replace(y, '');
                }
            };
        };
        var B = {
        ...
        j.add('list', {
            init: function(E) {
                var F = E.addCommand('numberedlist', new w('numberedlist', 'ol')),
                G = E.addCommand('bulletedlist', new w('bulletedlist', 'ul'));
    ...
    j.add('resize', {
        ...
        j.scayt = {
            ...
            loadEngine: function(t) {
                ...
                var v = 'svc.spellchecker.net/scayt26/loader__base.js',
                ...
                function() {
                    ...
                    function U(V) {
                        ...
                        if (c) {
                            ...
                            C.config.enterMode != 1 && W.on('selectionchange',
                            function() {
                                var af = W.getBody(),
        ...
        R = function(S) {
            ...
            k.dialog = {
                uiElement: function(S, T, U, V, W, X, Y) {
                    ...
                    S.on('load',
                    function() {
                        if (am.getInputElement()) am.getInputElement().on('focus',
                        function() {
                ...
                return function(z) {
                    ...
                    if (z) {
                        ...
                        for (var C = 0; C < B.length; C++) {
                            ...
                            if (F && F.type == 3) if (H >= F.getLength()) J.setStartAfter(F);
